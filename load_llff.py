...
# compute spiral poses for optoimized poses
def render_path_optimized(poses):
    poses = poses.detach().cpu().numpy()
    c2w = poses_avg(poses)
    up = normalize(poses[:, :3, 1].sum(0))
    close_depth, inf_depth = 0.1 * .9, 1 * 5.
    dt = .75
...
def render_path_spiral(c2w, up, rads, focal, zdelta, zrate, rots, N):
    ...
    for theta in np.linspace(0., 2. * np.pi * rots, N + 1)[:-1]:
        ...
        z = normalize(c - np.dot(c2w[:3, :4], np.array([0, 0, -10*focal, 1.])))
